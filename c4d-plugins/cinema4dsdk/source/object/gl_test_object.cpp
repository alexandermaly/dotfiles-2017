...
Bool GLTestObject::DrawParticles(BaseObject* pObject, BaseDraw* bd, BaseDrawHelp* bh, Int32 lType)
{
	...
	if (!g_pvParticlePoint || !g_pvParticleColor || !g_pvParticleNormal)
	{
		...
		g_pvParticlePoint	 = NewMem(Vector32, lPoints);
		g_pvParticleColor	 = NewMem(Vector32, lPoints);
		g_pvParticleNormal = NewMem(Vector32, lPoints);
		if (!g_pvParticlePoint || !g_pvParticleColor || !g_pvParticleNormal)
			return false;
	...
	if (lType == GLTEST_TYPE_PARTICLE_FAST_BLOCK || lType == GLTEST_TYPE_PARTICLE_FAST_BUFFER)
	{
		...
		if (lDrawportType == DRAWPORT_TYPE_OGL_HQ && lType == GLTEST_TYPE_PARTICLE_FAST_BUFFER)
		{
			...
			if (lLightCount > 0)
			{
				ppLights = NewMemClear(const GlLight *, lLightCount);
...
Bool RegisterGLTestObject()
{
	...
	g_pVectorInfo = NewObj(maxon::BaseArray<GlVertexBufferVectorInfo>);
	g_pAttributeInfo = NewObj(maxon::BaseArray<GlVertexBufferAttributeInfo>);
	if (!g_pVectorInfo || !g_pAttributeInfo || !g_pVectorInfo->Resize(3) || !g_pAttributeInfo->Resize(3))
		return false;
	...
	return RegisterObjectPlugin(450000131, name, 0, GLTestObject::Alloc, "ogltest", nullptr, 0);
}
